#!/usr/bin/env python
"""
Compare diffusion samples from VAE vs KAE on Moving MNIST.
...
def load_vae_from_checkpoint(checkpoint_path: str, latent_dim: int = 64) -> VideoVAE:
...
def load_diffusion_with_encoder(
    ...
    # For Moving MNIST with latent_dim=64: reshaped to [4, 4, 4] (C=4, H=4, W=4)
...
def save_comparison_grid(
    ...
    plt.suptitle('Moving MNIST: Diffusion Generated Videos', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def save_multi_sample_grid(
    ...
    plt.suptitle('Moving MNIST: Multiple Diffusion Samples Comparison', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Saved: {output_path}")
...
def main():
    ...
    parser.add_argument("--kae_diffusion_checkpoint", type=str,
                        default="outputs/checkpoints/diffusion_kae_compare/diffusion-kae-compare-epoch=296-train_loss=0.1068.ckpt")
    parser.add_argument("--vae_diffusion_checkpoint", type=str,
                        default="outputs/checkpoints/diffusion_vae_compare/diffusion-vae-compare-epoch=251-train_loss=0.0398.ckpt")
    parser.add_argument("--kae_checkpoint", type=str,
                        default="/private/home/soniajoseph/IFT-4031/koopmanAE/results/trained_models_mnist/focal_dice_single_frame.pth")
    parser.add_argument("--vae_checkpoint", type=str,
                        default="outputs/checkpoints/vae_movingmnist/last.ckpt")
    parser.add_argument("--data_path", type=str,
                        default="/datasets01/MovingMNIST")
    parser.add_argument("--output_dir", type=str,
                        default="outputs/visualizations/mnist_comparison")
    ...
    print("Loading Moving MNIST data...")
    ...
    kae = load_koopman_ae_from_checkpoint(
        ...
        latent_dim=64, input_channels=1, image_size=64,
        dataset_name="MovingMNIST", input_frames=1, pred_frames=5,
        device=args.device,
    )
    kae.eval()
    ...
    if not kae_ckpt.exists():
        ckpt_dir = Path("outputs/checkpoints/diffusion_kae_compare")
    ...
    if not vae_ckpt.exists():
        ckpt_dir = Path("outputs/checkpoints/diffusion_vae_compare")
        ckpts = list(ckpt_dir.glob("*.ckpt"))
        if ckpts:
