...
def main(checkpoint_path: str, output_dir: str, num_samples: int = 4):
    """Generate visualizations from VAE checkpoint."""
    ...
    for i in range(num_samples):
        ...
        save_video_comparison_gif(
            orig, recon,
            str(output_dir / f"vae_sample_{i}_comparison.gif"),
        )

        # Save frame grid
        save_frame_grid(
            orig, recon,
            str(output_dir / f"vae_sample_{i}_frames.png"),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to VAE checkpoint (.ckpt file)",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
